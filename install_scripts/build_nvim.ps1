...
# 3N4N
$env:DEPS_BUILD_DIR = [System.IO.Path]::GetFullPath("E:/projects/neovim/deps_msvc/")
$env:DEPS_PREFIX = Join-Path -Path $env:DEPS_BUILD_DIR -ChildPath 'usr'
$buildDir = Join-Path -Path $projectDir -ChildPath "release"
Write-Host "DEPS_BUILD_DIR" $env:DEPS_BUILD_DIR
Write-Host "DEPS_PREFIX" $env:DEPS_PREFIX
...
Write-Host "depsCmakeVars" $(convertToCmakeArgs($depsCmakeVars))
Write-Host "nvimCmakeVars" $(convertToCmakeArgs($nvimCmakeVars))
Write-Host "vspath" $installationPath
